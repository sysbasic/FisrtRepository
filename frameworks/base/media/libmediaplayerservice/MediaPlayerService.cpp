...
static player_type getDefaultPlayerType_Decode() {
#if BUILD_WITH_FULL_STAGEFRIGHT
    char value[PROPERTY_VALUE_MAX];
    if (property_get("media.stagefright.enable-player", value, NULL)
        && (!strcmp(value, "1") || !strcasecmp(value, "true"))) {
...
#if BUILD_WITH_CIPLAYER  // aac
	if (ident == 0x46494441) // 'ADIF'
	{
		return CI_PLAYER;
	}

	// adts
	unsigned int syncWord = (buf[0] << 4) | (buf[1] >> 4);
	if (syncWord == 0xfff)
	{
		unsigned char layer = (buf[1] >> 1) & 0x3;
		if (layer == 0)
		{
			return CI_PLAYER;
		}
...
status_t MediaPlayerService::Client::setVolume(float leftVolume, float rightVolume)
{
	...
	// hardware output
    {
		...
		if (p != 0)
		{
			MediaPlayerBase * basePlayer = p.get();
			if (basePlayer != NULL)
			{
				//MediaPlayerHWInterface * hwPlayer = dynamic_cast<MediaPlayerHWInterface *>(basePlayer);
				MediaPlayerHWInterface * hwPlayer = (MediaPlayerHWInterface *)basePlayer;
				if (hwPlayer != NULL)
				{
					return hwPlayer->setVolume(leftVolume, rightVolume);
				}
			}
		}
    }
...
sp<IMemory> MediaPlayerService::decode(const char* url, uint32_t *pSampleRate, int* pNumChannels, int* pFormat)
{
    ...
    player_type playerType = getDefaultPlayerType_Decode();//getPlayerType(url);
    LOGV("player type = %d", playerType);
...
sp<IMemory> MediaPlayerService::decode(int fd, int64_t offset, int64_t length, uint32_t *pSampleRate, int* pNumChannels, int* pFormat)
{
    ...
    player_type playerType = getDefaultPlayerType_Decode();//getPlayerType(fd, offset, length);
    LOGV("player type = %d", playerType);
