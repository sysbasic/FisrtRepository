...
public class CDMAPhone extends PhoneBase {
    ...
    private native void  nativeEnablePower(boolean on);
    public void  enablePower(boolean on) {
        nativeEnablePower(on);
        Settings.Secure.putInt(mContext.getContentResolver(), Settings.System.MPCIE_SETTING, on?1:0);
        broadcast3GKeepAwaken();
    }
    public boolean isEnabledPower() {
        int def_val = SystemProperties.getInt("ro.livall.def.3gpower", 0);
        boolean enable = (Settings.Secure.getInt(mContext.getContentResolver(), Settings.System.MPCIE_SETTING, (def_val&0x01)) != 0);
        return enable;
    }
    public void keep3GAwaken(boolean keep) {
        Settings.Secure.putInt(mContext.getContentResolver(), Settings.System.MPCIE_KEEP_AWAKEN, keep?1:0);
        broadcast3GKeepAwaken();
    }
    public boolean isKeep3GAwanek() {
        int def_val = SystemProperties.getInt("ro.livall.def.3gpower", 0);
        boolean enable = (Settings.Secure.getInt(mContext.getContentResolver(), Settings.System.MPCIE_KEEP_AWAKEN, ((def_val&0x02)==0x02)?1:0) != 0);
        return enable;
    }
    private void broadcast3GKeepAwaken() {
        Intent intent = new Intent(TelephonyIntents.ACTION_KEEP_3G_AWAKEN_CHANGED);
        intent.putExtra(Settings.System.MPCIE_KEEP_AWAKEN, isEnabledPower()&&isKeep3GAwanek());
        ActivityManagerNative.broadcastStickyIntent(intent,null);
    }

    private native int nativeGet3GSupportType();
    public int get3GSupportType() {
        return nativeGet3GSupportType();
    }
    ...
    public CDMAPhone(Context context, CommandsInterface ci, PhoneNotifier notifier,
        ...
        System.loadLibrary("telephonycdma");
