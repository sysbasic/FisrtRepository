...
//ProcessState是一个singleton类型，一个进程只能创建一个他的对象。他的作用是维护当前进程中所有Service代理（BpBinder对象）。
//一个客户端进程可能需要多个Service的服务，这样可能会创建多个Service代理（BpBinder对象），客户端进程中的ProcessState对象将会负责维护这些Service代理。
    ...
    , mThreadPoolSeq(1)
{
    if (mDriverFD >= 0) {
        ...
        //BIDNER_VM_SIZE定义为(1*1024*1024) - (4096 *2)  即: 1M-8K
        //这个需要你自己去man mmap的用法了，不过大概意思就是将fd映射为内存，这样内存的memcpy等操作就相当于write/read(fd)了
