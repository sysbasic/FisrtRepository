...
public abstract class ContentProvider implements ComponentCallbacks {
    ...
    class Transport extends ContentProviderNative {
        ...
        public int bulkDelete(Uri[] uris, String selection, String[] selectionArgs) {
            for(int i=0; i< uris.length; i++)
                enforceWritePermission(uris[i]);

            return ContentProvider.this.bulkDelete(uris, selection, selectionArgs);
        }
     ...
     * responsible for parsing the record number (22) when creating a SQL statement.
     *
     * @param uris The full URI to query, including a row ID (if a specific record is requested).
     * @param selection An optional restriction to apply to rows when deleting.
     * @return The number of rows affected.
     * @throws SQLException
     */
    public int bulkDelete(Uri[] uris, String selection, String[] selectionArgs) {
        int numUris = uris.length;

        for (int i = 0; i < numUris; i++) {
            delete(uris[i], selection, selectionArgs);
        }
        return numUris;
    }
