...
public class WifiManager {
    ...
    public boolean setWifiEnabled_ex(boolean enabled) {
        try {
            return mService.setWifiEnabled_ex(enabled);
        } catch (RemoteException e) {
            return false;
        }
    }
    ...
    /* @hide boolean checkPermission*/
    public int getWifiState_ex() {
        try {
            return mService.getWifiEnabledState_ex();
        } catch (RemoteException e) {
            return WIFI_STATE_UNKNOWN;
        }
    }
    ...
    public static int calculateSignalLevel(int rssi, int numLevels) {
        if (rssi <= MIN_RSSI) {
            ...
            return (rssi - MIN_RSSI) / partitionSize + 1;
        }
    }
