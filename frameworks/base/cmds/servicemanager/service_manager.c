...
//我们看到首先检查是否有权限注册service，没权限就对不起了，出错返回；然后检查是否已经注册过，注册过的service将不能再次注册。
//然后构造一个svcinfo对象，并加入一个全局链表中svclist中。最后通知binder设备：有一个service注册进来。
...
//    LOGI("add_service('%s',%p) uid=%d\n", str8(s), ptr, uid);
    ...
    if (!svc_can_register(uid, s)) {  //验证权限
