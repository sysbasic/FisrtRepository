...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private void loadSecureSettings(SQLiteDatabase db) {
        ...
        try {
            ...
            loadSetting(stmt, Settings.Secure.MOBILE_DATA,
                    "true".equalsIgnoreCase(SystemProperties.get("ro.com.android.mobiledata", "false")) ? 1 : 0);
            // Data roaming default, based on build
            loadSetting(stmt, Settings.Secure.DATA_ROAMING,
                    "true".equalsIgnoreCase(SystemProperties.get("ro.com.android.dataroaming", "false")) ? 1 : 0);

            boolean defaultValue = SystemProperties.getBoolean("livall.non_market_app.default", false);
            loadSetting(stmt, Settings.Secure.INSTALL_NON_MARKET_APPS,
                    (defaultValue || mContext.getResources().getBoolean(R.bool.def_install_non_market_apps)) ? 1 : 0);
    ...
    //2011-08-16 hengai: If settings.db bad, write default key-value
    private void loadSecureOtherSettings(SQLiteDatabase db) {        
        SQLiteStatement stmt = null;
        String prop = "persist.sys.settingsdb";
        int other_settings = SystemProperties.getInt(prop, 0);
        SystemProperties.set(prop, Integer.toString(other_settings+1));
        if(other_settings == 0) {   //is first create settings.db
            return;
        }
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO secure(name,value)"
