...
public class StatusBarService extends IStatusBar.Stub implements OnTouchListener
{
    ...
    private void makeStatusBarView(Context context) {
        ...
        mPlmnLabel.setText(R.string.lockscreen_carrier_default_fuhu);
    ...
    public void systemReady() {
        ...
        if (mDisplay.getSystemMenuWidth() > 0) {
    ...
    void doAnimation() {
        if (mAnimating) {
            ...
            else if (mAnimY < mStatusBarView.getHeight() + mDisplay.getHdmiTop()) {
    ...
    boolean interceptTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (!mExpanded) {
                ...
                mViewDelta = mAbsPos[1] + mTrackingView.getHeight() - y - mDisplay.getHdmiTop();
            }
            Slog.d(TAG, "mExpanded=" + mExpanded + " statusBarSize=" + statusBarSize + " mViewDelta="+ mViewDelta + " mTrackingView=" + mTrackingView.getHeight());
            if ((!mExpanded && y < hitSize + mDisplay.getHdmiTop()) ||
                    (mExpanded && y > (mDisplay.getHeight() + mDisplay.getHdmiTop() -hitSize))) {
            ...
            Slog.d(TAG, "x=" + x + " edgeBorder=" + edgeBorder );
            ...
            Slog.d(TAG, "y=" + y + " minY=" + minY  + " mAnimatingReveal=" + mAnimatingReveal  + " mViewDelta=" + mViewDelta );
...
//        lp.token = mStatusBarView.getWindowToken();
    ...
    void updateExpandedViewPos(int expandedPosition) {
        ...
        if (!mExpandedVisible) {
            if (mTrackingView != null) {
                ...
                if (mTrackingParams != null) {
                    //mTrackingParams.x = mDisplay.getLeft();
                    mTrackingParams.y = mTrackingPosition;// + mDisplay.getHdmiTop();
                    WindowManagerImpl.getDefault().updateViewLayout(mTrackingView, mTrackingParams);
                }
            }
            if (mExpandedParams != null) {
                //mExpandedParams.x = mDisplay.getLeft();
                mExpandedParams.y = -disph;//  + mDisplay.getHdmiTop();
                mExpandedDialog.getWindow().setAttributes(mExpandedParams);
            }
            return;
        }
        ...
        if (expandedPosition == EXPANDED_FULL_OPEN) {
            pos = h;// + mDisplay.getHdmiTop();
        }
        else if (expandedPosition == EXPANDED_LEAVE_ALONE) {
            pos = mTrackingPosition;// + mDisplay.getHdmiTop();
        }
        else {
            if (expandedPosition <= disph) {
                pos = expandedPosition;// - mDisplay.getHdmiTop();
            } else if (expandedPosition <= disph + mDisplay.getHdmiTop()) {
                pos = expandedPosition - mDisplay.getHdmiTop();
            } else {
                pos = disph;// + mDisplay.getHdmiTop();
            }
            pos -= disph-h;
        }
