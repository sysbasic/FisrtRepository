...
public class UsbStorageActivity extends Activity
    ...
    private boolean mIsMassEnable = false;
    ...
    private boolean getUsbStorageInUse() {
        boolean on = false;
        int mediaAvaiableCount = 0;
        int mediaSharedCount = 0;
        if(localLOGV) {
            Log.d(TAG, "getVolume(NAND::" + Environment.getNandStorageState()+ ")");
            Log.d(TAG, "getVolume(SDCARD::" + Environment.getSdcardStorageState()+ ")");
            Log.d(TAG, "getVolume(SCSI::" + Environment.getScsiStorageState()+ ")");
        }
        String nandState = Environment.getNandStorageState();
        String sdcardState = Environment.getSdcardStorageState();
        String scsiState = Environment.getScsiStorageState();        

        // get count of shared media 
        if( Environment.MEDIA_SHARED.equals(nandState)) {
            mediaSharedCount++;
        }

        if( Environment.MEDIA_SHARED.equals(sdcardState)) {
            mediaSharedCount++;
        }

        if( Environment.MEDIA_SHARED.equals(scsiState)) {
            mediaSharedCount++;
        }

        // get count of available media
        if( !Environment.MEDIA_REMOVED.equals(nandState)) {
            mediaAvaiableCount++;
        }
        ...
        if( !Environment.MEDIA_REMOVED.equals(scsiState)) {
            mediaAvaiableCount++;
        }

        if( mediaAvaiableCount == 0 ) {
            on = false;
            mIsMassEnable = false;
        } else {
            if( mIsMassEnable ) {
                if( mediaAvaiableCount == mediaSharedCount ) {
                    on = true;
                } else {
                    on = false;
                }
    ...
    private StorageEventListener mStorageListener = new StorageEventListener() {
        ...
        public void onStorageStateChanged(String path, String oldState, String newState) {
            ...
            /*hengai*/
            boolean on = getUsbStorageInUse();

            if( Environment.MEDIA_CHECKING.equals(newState) ) {
                mIsMassEnable = false;
            } else if( Environment.MEDIA_SHARED.equals(newState) ) {
                mIsMassEnable = true;
            }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mIsMassEnable = mStorageManager.isUsbMassStorageConnected();
    ...
    private void switchDisplay(boolean usbStorageInUse) {
        if(localLOGV) Log.d(TAG, "mIsMassEnable=" + mIsMassEnable + " newUsbStorageInUse=" + usbStorageInUse);
        
        if( mIsMassEnable != usbStorageInUse ) {
            return;
        }
        ...
        if (usbStorageInUse) {
            ...
            /* hengai */
            mBanner.setText(com.android.internal.R.string.usb_nand_storage_title);
    ...
    private void switchUsbMassStorageAsync(boolean on) {
        if(localLOGV) Log.i(TAG, "switchUsbMassStorageAsync:  " + on);
        mUnmountButton.setVisibility(View.GONE);
        mMountButton.setVisibility(View.GONE);
    ...
    private void checkStorageUsers() {
        ...
        if( Environment.isMounted(MediaStore.NAND_VOLUME)) {
            String storagePath = Environment.getNandStorageDirectory().toString();
            
            try {
                int[] stUsers = ims.getStorageUsers(storagePath);
        ...
        if( !showDialog && Environment.isMounted(MediaStore.SDCARD_VOLUME)) {

            String storagePath = Environment.getSdcardStorageDirectory().toString();
        ...
        if( !showDialog && Environment.isMounted(MediaStore.SCSI_VOLUME)) {

            String storagePath = Environment.getScsiStorageDirectory().toString();
    ...
    public void onClick(View v) {
        if (localLOGV) Log.i(TAG, "onClick: UMS Enable/Disable");    
        if (v == mMountButton) {
