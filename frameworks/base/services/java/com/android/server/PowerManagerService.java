...
class PowerManagerService extends IPowerManager.Stub
    ...
    private int mAsynPartialCount = 0;
    ...
    private boolean mbScreenOff = false;
    private boolean mUnloadWifiDriver = false;
    ...
    */
    ...
    private class UnsynchronizedWakeLock {
        ...
        public void acquire() {
            if (!mRefCounted || mCount++ == 0) {
                ...
                try {
                    PowerManagerService.this.acquireWakeLockLocked(mFlags, mToken, MY_UID, mTag, false);
                    mHeld = true;
                } finally {
    ...
    {
        WakeLock(int f, IBinder b, String t, int u, boolean synchronize) {
            super();
            flags = f;
            binder = b;
            tag = t;
        ...
        public void binderDied() {
            synchronized (mLocks) {
                ...
                if (synch) {
                    mAsynPartialCount--;
                    if (mAsynPartialCount == 0 && mbScreenOff ) {
                        WifiManager mWM = (WifiManager)mContext.getSystemService(Context.WIFI_SERVICE);
                        //Slog.d(TAG, "=====zkl===  binderDied2   WifiState:" + mWM.getWifiState() + " WifiDriver:" + mUnloadWifiDriver);
                        if ((mWM.getWifiState_ex() == WifiManager.WIFI_STATE_ENABLING || mWM.getWifiState_ex() == WifiManager.WIFI_STATE_ENABLED)
                                && !mUnloadWifiDriver ) {
                            Slog.d(TAG, "=====zkl==== binderDied3  setWifiEnabled(false)");
                            mWM.setWifiEnabled_ex(false);
                            mUnloadWifiDriver = true;
                        }
                    }
                }
    ...
    public void acquireWakeLock(int flags, IBinder lock, String tag) {
        ...
        try {
            synchronized (mLocks) {
                ...
                acquireWakeLockLocked(flags, lock, uid, tag, true);

                if (index < 0) {
                    WakeLock wl = mLocks.get(mLocks.getIndex(lock));
                    if ((wl.flags & LOCK_MASK) == PowerManager.PARTIAL_WAKE_LOCK) {
                        mAsynPartialCount++;
                    }
                    //Slog.d(TAG, "=====zkl===  acquireWakeLock   PC=" + mPartialCount + "APC=" + mAsynPartialCount + " tag=" + wl.tag + " synch=" + wl.synch  + " wl.flags=" + (wl.flags & LOCK_MASK) );
                }
    ...
    public void acquireWakeLockLocked(int flags, IBinder lock, int uid, String tag, boolean synch) {
        int acquireUid = -1;
        ...
        boolean newlock;
        if (index < 0) {
            wl = new WakeLock(flags, lock, tag, uid, synch);
        ...
        //Slog.d(TAG, "=====zkl===  acquireWakeLockLocked    PC=" + mPartialCount + "APC=" + mAsynPartialCount + " tag=" + tag + " synch=" + synch);
    ...
    public void releaseWakeLock(IBinder lock, int flags) {
        ...
        /*  zkl, if screen off have not any PARTIAL_WAKE_LOCK, then never call this code */
        ...
        if (index >= 0) {
            ...
            //Slog.d(TAG, "=====zkl===  releaseWakeLock2  tag=" + wl.tag + " synch=" + wl.synch + " wl.flags=" + (wl.flags & LOCK_MASK));
            if ((wl.flags & LOCK_MASK) == PowerManager.PARTIAL_WAKE_LOCK) {
                mAsynPartialCount--;
                //Slog.d(TAG, "=====zkl===  releaseWakeLock3  PC=" + mPartialCount + " APC=" + mAsynPartialCount + " mbScreenOff=" + mbScreenOff);
                if (mAsynPartialCount == 0 && mbScreenOff ) {

                    //mContext.enforceCallingOrSelfPermission(android.Manifest.permission.WRITE_SETTINGS, null);

                    WifiManager mWM = (WifiManager)mContext.getSystemService(Context.WIFI_SERVICE);
                    //Slog.d(TAG, "=====zkl===  releaseWakeLock4   WifiState:" + mWM.getWifiState() + " WifiDriver:" + mUnloadWifiDriver);
                    //if (mWM.isWifiEnabled() && !mUnloadWifiDriver ) {
                    if ((mWM.getWifiState_ex() == WifiManager.WIFI_STATE_ENABLING || mWM.getWifiState_ex() == WifiManager.WIFI_STATE_ENABLED)
                            && !mUnloadWifiDriver ) {
                        Slog.d(TAG, "=====zkl=========  releaseWakeLock5    setWifiEnabled(false)");
                        mWM.setWifiEnabled_ex(false);
                        mUnloadWifiDriver = true;
                    }
                    //mContext.enforceCallingOrSelfPermission(android.Manifest.permission.WRITE_SETTINGS, null);
        ...
        //Slog.d(TAG, "=====zkl===  releaseWakeLock6    PC=" + mPartialCount + "APC=" + mAsynPartialCount + " SO= " + mbScreenOff );
    }
    ...
    private void releaseWakeLockLocked(IBinder lock, int flags, boolean death) {
        ...
        if (isScreenLock(wl.flags)) {
            ...
            mPartialCount--;
            //Slog.d(TAG, "=====zkl===  releaseWakeLockLocked   PC=" + mPartialCount + "APC=" + mAsynPartialCount + " tag=" + wl.tag + " synch=" + wl.synch );
            ...
            while (true) {
                ...
                if (value == 1) {
                    ...
                    if (mContext != null && ActivityManagerNative.isSystemReady()) {
                        /*  zkl */
                        mbScreenOff = false;
                        if (mUnloadWifiDriver) {
                            Slog.d(TAG, "=====zkl========  mScreenOnIntent       setWifiEnabled(true)" );
                            WifiManager mWM = (WifiManager)mContext.getSystemService(Context.WIFI_SERVICE);
                            mWM.setWifiEnabled_ex(true);
                            mUnloadWifiDriver = false;
                        }
                    ...
                    if (mContext != null && ActivityManagerNative.isSystemReady()) {
                        ...
                        //Slog.d(TAG, "=====zkl=====mNotificationTask===mUnloadWifiDriver=" +  mUnloadWifiDriver  + " mPartialCount=" + mPartialCount);
                        if (!mUnloadWifiDriver) {
                            // 
                            PowerManager powerManager = (PowerManager)mContext.getSystemService(Context.POWER_SERVICE);
                            mWakeLock = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "sScreenOffLock");
                            mWakeLock.acquire();

                            Slog.d(TAG, "=====zkl========mNotificationTask  mScreenOffIntent PC=" + mPartialCount + "APC=" + mAsynPartialCount );
                            mWakeLock.release();
                        }
    ...
    /**
     * 2011-08-11 Light(leds) control interface
     * onMS: 
     *      0:off
     *      <0: on always
     *      >0:on msec time
     * {@hide}
     */
    public void setLightValue(int id, int onMS) {
        LightsService.Light light;
        //Log.w(TAG, String.format("PowerManagerService setLightValue(%d,%d) ...", id, onMS));
        switch(id) {
            case LightsService.LIGHT_ID_BACKLIGHT:
                light = mLcdLight;
                break;
            case LightsService.LIGHT_ID_BUTTONS:
                light = mButtonLight;
                break;
            case LightsService.LIGHT_ID_KEYBOARD:
                light = mKeyboardLight;
                break;
            case LightsService.LIGHT_ID_ATTENTION:
        ...
        if(onMS == 0) {
            ...
            light.setBrightness(0x000000ff);
        }
    }
}
