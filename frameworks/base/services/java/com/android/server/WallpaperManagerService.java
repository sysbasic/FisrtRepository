...
class WallpaperManagerService extends IWallpaperManager.Stub {
    ...
    static boolean mIsKidsMode;
    static final File WALLPAPER_DIR = new File(
            "/data/data/com.android.settings/files");
    static final String WALLPAPER_MOM = "wallpaper";
    static final String WALLPAPER_KID = "kidmodewallpaper";
    static final File WALLPAPER_FILE_MOM = new File(WALLPAPER_DIR, WALLPAPER_MOM);
    static final File WALLPAPER_FILE_KID = new File(WALLPAPER_DIR, WALLPAPER_KID);
    static File WALLPAPER_FILE;
    ...
    private BroadcastReceiver mModeChangeIntentReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            String nabimode = intent.getStringExtra("com.nabi.intent.extra.MODE_NAME");

            if (nabimode.equals("com.nabi.intent.extra.mode.MOMMY_MODE")) {
                WALLPAPER_FILE = WALLPAPER_FILE_MOM;
                mIsKidsMode = false;
                mWallpaperComponent = mWallpaperComponent_kid;
                mNextWallpaperComponent = mNextWallpaperComponent_kid;
            } else if (nabimode.equals("com.nabi.intent.extra.mode.KID_MODE")) {            
                WALLPAPER_FILE = WALLPAPER_FILE_KID;
                mIsKidsMode = true;
                mWallpaperComponent = mWallpaperComponent_mom;
                mNextWallpaperComponent = mNextWallpaperComponent_mom;
            } else {
                return;
            }
            notifyCallbacksLocked();
    ...
    ComponentName mWallpaperComponent_mom;
    ComponentName mWallpaperComponent_kid;
    
    /**
     * The component name of the wallpaper that should be set next.
     */
    ComponentName mNextWallpaperComponent;
    ComponentName mNextWallpaperComponent_mom;
    ComponentName mNextWallpaperComponent_kid;
    ...
    WallpaperConnection mWallpaperConnection_kid;
    WallpaperConnection mWallpaperConnection_mom;
    long mLastDiedTime;
    ...
    public WallpaperManagerService(Context context) {
        ...
        mIsKidsMode = !("com.nabi.intent.extra.mode.MOMMY_MODE").equals(SystemProperties.get("livall.nabimode")) ;
        WALLPAPER_FILE = mIsKidsMode ? WALLPAPER_FILE_KID : WALLPAPER_FILE_MOM;
        loadSettingsLocked();
    ...
    public void systemReady() {
        ...
        filter.addAction("com.nabi.intent.action.MODE_CHANGE");
        mContext.registerReceiver(mModeChangeIntentReceiver, filter);        
    }
    ...
    public void setDimensionHints(int width, int height) throws RemoteException {
        ...
        if(mIsKidsMode) {
            synchronized (mLock) {
                if (width != mWidth_nabi || height != mHeight_nabi) {
                    mWidth_nabi= width;
                    mHeight_nabi= height;
                    saveSettingsLocked();
    ...
    public int getWidthHint() throws RemoteException {
        synchronized (mLock) {
            return mIsKidsMode ? mWidth_nabi : mWidth;
        }
    }

    public int getHeightHint() throws RemoteException {
        synchronized (mLock) {
            return mIsKidsMode ? mHeight_nabi : mHeight;
        }
    }
    ...
    public ParcelFileDescriptor getWallpaper(IWallpaperManagerCallback cb,
        ...
        synchronized (mLock) {
            try {
                if (outParams != null) {
                    outParams.putInt("width", mIsKidsMode ? mWidth_nabi : mWidth);
                    outParams.putInt("height", mIsKidsMode ? mHeight_nabi : mHeight);
                }
    ...
    void bindWallpaperComponentLocked(ComponentName componentName) {
        ...
        try {
            ...
            clearWallpaperComponentLocked();
            if (mIsKidsMode) {
                mWallpaperComponent_kid = componentName;
            } else {
                mWallpaperComponent_mom = componentName;
            }            
    ...
    void attachServiceLocked(WallpaperConnection conn) {
        try {
            conn.mService.attach(conn, conn.mToken,
                    ...
                    mIsKidsMode ? mWidth_nabi : mWidth, mIsKidsMode ? mHeight_nabi : mHeight);
        } catch (RemoteException e) {
    ...
    private void saveSettingsLocked() {
        ...
        try {
            ...
            if (mWallpaperComponent_mom != null) {
                out.attribute(null, "component",
                        mWallpaperComponent_mom.flattenToShortString());
            }
            out.endTag(null, "wp");

            //write wallpaper of kidsmode
            out.startTag(null, "wp-kidsmode");            
            out.attribute(null, "width", Integer.toString(mWidth_nabi));
            out.attribute(null, "height", Integer.toString(mHeight_nabi));
            out.attribute(null, "name", mName_nabi);            
            if (mWallpaperComponent_kid != null) {
                out.attribute(null, "component",
                        mWallpaperComponent_kid.flattenToShortString());
            }
            out.endTag(null, "wp-kidsmode");

            out.endTag(null, "wallpapers");
        ...
        try {
            ...
            do {
                ...
                if (type == XmlPullParser.START_TAG) {
                    ...
                    if ("wp-kidsmode".equals(tag)) {
                        mWidth_nabi= Integer.parseInt(parser.getAttributeValue(null, "width"));
                        mHeight_nabi= Integer.parseInt(parser.getAttributeValue(null, "height"));
                        mName_nabi= parser.getAttributeValue(null, "name");                        
                        String comp = parser.getAttributeValue(null, "component");
                        mNextWallpaperComponent_kid = comp != null
                                ? ComponentName.unflattenFromString(comp)
                                : null;
                        mWallpaperComponent_kid = mNextWallpaperComponent_kid;
                          
                        if (DEBUG) {
                            Slog.v(TAG, "mWidth_nabi:" + mWidth_nabi);
                            Slog.v(TAG, "mHeight_nabi:" + mHeight_nabi);
                            Slog.v(TAG, "mName_nabi:" + mName_nabi);
                            Slog.v(TAG, "mNextWallpaperComponent_kid:" + mNextWallpaperComponent_kid);
                        }
                    }
                    ...
                    if ("wp".equals(tag)) {
                        ...
                        String comp = parser.getAttributeValue(null, "component");
                        mNextWallpaperComponent_mom = comp != null
                                ? ComponentName.unflattenFromString(comp)
                                : null;
                        mWallpaperComponent_mom = mNextWallpaperComponent_mom;

                        if (DEBUG) {
                            Slog.v(TAG, "mWidth:" + mWidth);
                            Slog.v(TAG, "mHeight:" + mHeight);
                            Slog.v(TAG, "mName:" + mName);
                            Slog.v(TAG, "mNextWallpaperComponent_mom:" + mNextWallpaperComponent_mom);
                        }
                    }
                    mNextWallpaperComponent = mIsKidsMode ? mNextWallpaperComponent_kid : mNextWallpaperComponent_mom;
                }
    ...
    void settingsRestored() {
        ...
        synchronized (mLock) {
            ...
            if (mNextWallpaperComponent != null && 
                ...
                // use the default.
                if ("".equals(mIsKidsMode ? mName_nabi : mName)) {
        ...
        if (!success) {
            Slog.e(TAG, "Failed to restore wallpaper: '" + (mIsKidsMode ? mName_nabi : mName) + "'");
            if(mIsKidsMode){
                mName_nabi = "";
            } else {
                mName = "";
    ...
    boolean restoreNamedResourceLocked() {
        if ((mIsKidsMode ? mName_nabi : mName) .length() > 4 && "res:".equals((mIsKidsMode ? mName_nabi : mName) .substring(0, 4))) {
            String resName = (mIsKidsMode ? mName_nabi : mName) .substring(4);
    ...
    protected void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        ...
        synchronized (mLock) {
            ...
            pw.print("  mName="); pw.println((mIsKidsMode ? mName_nabi : mName) );
