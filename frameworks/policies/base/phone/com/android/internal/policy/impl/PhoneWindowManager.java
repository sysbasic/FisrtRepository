...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static public final String LOCK_ACTION ="com.android.internal.policy.impl.action.LOCK_ACTION";

    static final int KEY_LOCKED_ID = com.android.internal.R.string.label_notification_id;
	...
	boolean isMuteByRemoteController = false;

    boolean mTouchKeyLock =false;
    
    boolean mSafeMode;
    WindowState mStatusBar = null;
    WindowState mSystemMenuBar = null;    
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        // register for nabimode change
        IntentFilter filter2 = new IntentFilter();
        filter2.addAction("com.nabi.intent.action.MODE_CHANGE");
        filter2.addAction("com.nabi.intent.action.OOBE_HIDDEN");        
        Intent intent2 = context.registerReceiver(mNabiModeReceiver, filter2);
        ...
        mButtonLedonMS = SystemProperties.getInt("ro.livall.btnled", 0);
    }
    ...
    public boolean interceptKeyTi(WindowState win, int code, int metaKeys, boolean down, 
        ...
        boolean kidModeOn = ("com.nabi.intent.extra.mode.KID_MODE").equals(SystemProperties.get("livall.nabimode") );//unicadd
        boolean isOOBEFinished = (("done").equals(SystemProperties.get("persist.sys.nabimode.oobe")));
        Log.i("u","isOOBEFinished="+isOOBEFinished);
        ...
        if (code == KeyEvent.KEYCODE_HOME) {
            ...
            if (kidModeOn) {
                ...
                //intent.setClassName("com.fuhu.foozkidslauncher", "com.fuhu.foozkidslauncher.Launcher");
                intent.setClassName("cn.livall.KaKaPlayer", "cn.livall.KaKaPlayer.KaKaPlayerActivity");            
    ...
    public void beginLayoutLw(int displayWidth, int displayHeight) {
        ...
        mDockLeft = mContentLeft = mCurLeft = d.getHdmiLeft();
        mDockTop = mContentTop = mCurTop = d.getHdmiTop();
        mDockRight = mContentRight = mCurRight = displayWidth + d.getHdmiLeft();
        mDockBottom = mContentBottom = mCurBottom = displayHeight + d.getHdmiTop();
        ...
        pf.left = df.left = vf.left = d.getHdmiLeft();
        pf.top = df.top = vf.top = d.getHdmiTop();
        pf.right = df.right = vf.right = displayWidth + d.getHdmiLeft();
        pf.bottom = df.bottom = vf.bottom = displayHeight + d.getHdmiTop();
        ...
        if (mStatusBar != null) {
            ...
            if (mStatusBar.isVisibleLw()) {
                ...
                if (DEBUG_LAYOUT) 
                    Log.v(TAG, "Status bar: mDockBottom=" + mDockBottom + " mContentBottom=" + mContentBottom + " mCurBottom=" + mCurBottom);
            }
        }
        if (d.getSystemMenuWidth() > 0) {
            if (displayWidth > displayHeight) {
                pf.left = df.left = vf.left = displayWidth + d.getHdmiLeft();
                pf.right = df.right = vf.right = displayWidth + d.getSystemMenuWidth() + d.getHdmiLeft();
            } else {
                pf.top = df.top = vf.top = displayHeight+ d.getHdmiTop();
                pf.bottom = df.bottom = vf.bottom = displayHeight + d.getSystemMenuWidth() + d.getHdmiTop();
            }    
    ...
    public void layoutWindowLw(WindowState win, WindowManager.LayoutParams attrs,
        ...
        if (attrs.type == TYPE_INPUT_METHOD) {
            ...
            if ((fl &
                ...
                if (attached != null) {
                    ...
                    pf.left = df.left = d.getHdmiLeft();
                    pf.top = df.top = d.getHdmiTop();
                    pf.right = df.right = mW  + d.getHdmiLeft();
                    pf.bottom = df.bottom = mH + d.getHdmiTop();
                    if ((sim & SOFT_INPUT_MASK_ADJUST) != SOFT_INPUT_ADJUST_RESIZE) {
                        cf.left = mDockLeft;
                        cf.top = mDockTop;
                        cf.right = mDockRight;
                ...
                pf.left = df.left = cf.left = d.getHdmiLeft();
                pf.top = df.top = cf.top = d.getHdmiTop();
                pf.right = df.right = cf.right = mW  + d.getHdmiLeft();
                pf.bottom = df.bottom = cf.bottom = mH + d.getHdmiTop();
                vf.left = mCurLeft;
                vf.top = mCurTop;
                vf.right = mCurRight;
                vf.bottom = mCurBottom;
        ...
        if (DEBUG_LAYOUT) Log.v(TAG, "Compute frame " + attrs.getTitle() + ": sim=#" + Integer.toHexString(sim)
                + " pf=" + pf.toShortString() + " df=" + df.toShortString() + " cf=" + cf.toShortString() + " vf=" + vf.toShortString());
        
        if (false) {
            if ("com.google.android.youtube".equals(attrs.packageName) && attrs.type == WindowManager.LayoutParams.TYPE_APPLICATION_PANEL) {
                if (true || localLOGV) 
                    Log.v(TAG, "Computing frame of " + win + ": sim=#" + Integer.toHexString(sim)
                        + " pf=" + pf.toShortString() + " df=" + df.toShortString() + " cf=" + cf.toShortString() + " vf=" + vf.toShortString());
            }
        }
        ...
        if (attrs.type == TYPE_INPUT_METHOD && !win.getGivenInsetsPendingLw()) {
            ...
            if (DEBUG_LAYOUT) 
                Log.v(TAG, "Input method: mDockBottom=" + mDockBottom + " mContentBottom=" + mContentBottom + " mCurBottom=" + mCurBottom);
        }
    }
    ...
    public int finishAnimationLw() {
        ...
        if (mStatusBar != null) {
            //if (localLOGV) 
                //Log.i("finishAnimationLw", "force=" + mForceStatusBar + " top=" + mTopFullscreenOpaqueWindowState);
            if (mForceStatusBar) {
                ...
                if (hideStatusBar) {
                    ...
                    if (mStatusBar.showLw(true)) changes |= FINISH_LAYOUT_REDO_LAYOUT;
                    if (mSystemMenuBar != null) {
                        if (mSystemMenuBar.showLw(true)) changes |= FINISH_LAYOUT_REDO_LAYOUT;
                    }
                }
            }
        }
...
*/
		...
		if (type == RawInputEvent.EV_KEY) {
            if(down && (mButtonLedonMS!=0) && (code == KeyEvent.KEYCODE_MENU || code ==KeyEvent.KEYCODE_BACK
                || code ==KeyEvent.KEYCODE_SEARCH || code ==KeyEvent.KEYCODE_HOME
                || code ==KeyEvent.KEYCODE_VOLUME_UP || code ==KeyEvent.KEYCODE_VOLUME_DOWN)
              ){
                if (down){
                    mPowerManager.setLightValue(2, mButtonLedonMS);
                }
            }

            if((event.keycode==KeyEvent.KEYCODE_TOUCHCALIBRATE)
                && down
                ) {
                //2011-05-05 hengai Touchscreen calibration is first priority, it also invoke when there is keyguardActive or PIN 
                try {
                    Intent intent = new Intent(); //getContext().getPackageManager().getLaunchIntentForPackage("cn.livall.x7pCalibration");
                    intent.setClassName("cn.livall.x7pCalibration", "cn.livall.x7pCalibration.x7pCalibration");
            ...
            code ==KeyEvent.KEYCODE_BACK || code ==KeyEvent.KEYCODE_SEARCH || code ==KeyEvent.KEYCODE_HOME){
				...
				if (down){
					...
					if (audioManager != null) {						
						audioManager.setStreamSolo(AudioManager.STREAM_ALARM, 
							(isMuteByRemoteController = !isMuteByRemoteController));
					}
				}	
			}
        }
    ...
    BroadcastReceiver mNabiModeReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {            
            if ("com.nabi.intent.action.MODE_CHANGE".equals(intent.getAction())) {
                String nabimode = intent.getStringExtra("com.nabi.intent.extra.MODE_NAME");
                SystemProperties.set("livall.nabimode", nabimode);
                Log.i("u", "broadcast received,mode = "+nabimode);
                StatusBarManager sm = (StatusBarManager) mContext.getSystemService(Context.STATUS_BAR_SERVICE);
                if (nabimode.equals("com.nabi.intent.extra.mode.KID_MODE")) {
                    sm.disable_ex(StatusBarManager.DISABLE_EXPAND|StatusBarManager.DISABLE_NOTIFICATION_ALERTS
                            |StatusBarManager.DISABLE_NOTIFICATION_TICKER|StatusBarManager.DISABLE_NOTIFICATION_ICONS);
                } else if (nabimode.equals("com.nabi.intent.extra.mode.MOMMY_MODE")) {
                    try {
                        Thread.sleep(300);
            ...
            } else if ("com.nabi.intent.action.OOBE_HIDDEN".equals(intent.getAction())) {
                SystemProperties.set("persist.sys.nabimode.oobe", "done");
                Log.i("u", "broadcast received,mode = OOBE_HIDDEN");
            }
        }
    };
    ...
    public boolean detectSafeMode() {
        try {
            if(0 == SystemProperties.getInt("ro.enable.safemode", 1)) {
                mSafeMode = false;
                return false;
            }
    ...
    public void systemReady() {
        ...
        android.os.SystemProperties.set("livall.nabimode", "com.nabi.intent.extra.mode.KID_MODE"); //unicadd
        android.os.SystemProperties.set("persist.sys.nabimode.oobe", "done"); //unicadd
        Log.i("u", "mode="+android.os.SystemProperties.get("livall.nabimode"));
        synchronized (mLock) {
            updateOrientationListenerLp();
            mSystemReady = true;
            mHandler.post(new Runnable() {
